...
public class CommonREST {
    ...
    private CommandAdvertisement advertisement = new CommandAdvertisement();
    private Speak speak = new Speak();

    @GET
    @Path("/cmd/{text}")
    @Consumes(MediaType.TEXT_PLAIN + ";charset=utf-8")
    public String cmd(@PathParam("text") String text) throws JMSException, URISyntaxException {

        log.info(i18n.message("rest.get.cmd.0", text));

        messaging = new JsonMessaging(UUID.randomUUID());
        messaging.broadcast("event.command", advertisement.set(text));
        messaging.close();
    ...
    @Path("/speak/{text}")
    @Consumes(MediaType.TEXT_PLAIN + ";charset=utf-8")
    public String speak(@PathParam("text") String text) throws JMSException, URISyntaxException {
